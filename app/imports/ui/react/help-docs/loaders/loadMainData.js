...
import { setDataLoading, setIsLayoutReady } from '/client/redux/actions/globalActions';
...
export default ({ dispatch }, onData) => {
  ...
  if (sub.ready()) {
    const helpDocs = HelpDocs.find({}, { reactive: false }).fetch();
    const helpSections = HelpSections.find({}, {
      sort: { index: 1 },
      reactive: false,
    }).fetch();
